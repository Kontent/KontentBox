...
// Windows Media .WMV
				...
				mediaId = "mediaId_"+new Date().getTime();	// Safari may not update iframe content with a static id.
                                if (navigator.plugins && navigator.plugins.length) {
                                        preload = '<object id="'+mediaId+'" standby="loading windows media..." type="video/x-ms-wmv" data="'+URL+'" width="'+mediaWidth+'" height="'+mediaHeight+'" /><param name="src" value="'+URL+'" /><param name="autoStart" value="'+options.autoplay+'" /></object>';
                                } else {
                                        preload = '<object id="'+mediaId+'" standby="loading windows media..." classid="CLSID:22D6f312-B0F6-11D0-94AB-0080C74C7E95" type="video/x-ms-wmv" data="'+URL+'" width="'+mediaWidth+'" height="'+mediaHeight+'" /><param name="filename" value="'+URL+'" /><param name="showcontrols" value="'+options.controller+'"><param name="autoStart" value="'+options.autoplay+'" /><param name="stretchToFit" value="true" /></object>';
                                }
				startEffect();
        // Inline element
// MP3, AAC
				...
				mediaType = options.isApple?'inline':'obj';
				mediaWidth = mediaWidth || options.defaultWidth;
				mediaHeight = mediaHeight || options.defaultHeight || "20px";
				if (URL.match(/tweetmic\.com|tmic\.fm/i)) {
					...
					URL = "http://media4.fjarnet.net/tweet/tweetmicapp-"+URL[4]+'.mp3';
                                ...
                                if(options.isApple){
                                    mediaHeight=options.defaultHeight=16;
                                    mediaWidth=options.defaultWidth=200;
                                    preload="<audio src='"+URL+"' "+(options.controller?"controls":"")+" style='width:'"+parseInt(mediaWidth)+"px;height:'"+parseInt(mediaHeight)+"px' ></audio>";
                                }
...
// DailyMotion
				...
				startEffect();
// MyspaceTV
                 } else if (URL.match(/myspace\.com\/video/i)) {
                        mediaType = 'obj';
                        mediaWidth = mediaWidth || "320px";
                        mediaHeight = mediaHeight || "240px";
                        mediaSplit = URL.match(/([0-9]+)$/);
...
//			if (options.overflow) image.setStyles({overflow: options.overflow});
		...
		image.setStyles({width: mediaWidth, height: mediaHeight,opacity: options.resizeOpening ? 0 : 1,visibility:'visible'});
...
//	$$('#mb_').each(function(hide) { hide.set('display', 'none'); });
	var links = $$("a").filter(function(el) {
		return el.rel && el.rel.test(/^mediabox|^lightbox/i);
	});
